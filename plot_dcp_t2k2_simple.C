...
void plot_dcp_t2k2_simple(){
    ...
    new TCanvas;
    ci = TColor::GetColor(colorcode[0]);
    hhist[1][1]->SetLineColor(ci);
    
    ci = TColor::GetColor(colorcode[1]);
    hhist[1][2]->SetLineColor(ci);
    hhist[1][2]->GetYaxis()->SetRangeUser(0,12);
    hhist[1][2]->GetYaxis()->SetTitle("#sqrt{#Delta #chi^{2}}");
    hhist[1][2]->GetXaxis()->SetTitle("Test values of #delta_{CP} [rad]");
    titleStyle(hhist[1][2]);
    hhist[1][2]->Draw("hist");
    hhist[1][1]->Draw("hist same");
    TLegend* leg1 = new TLegend(.22, .62, 0.42, .92);
    leg1->SetFillStyle(0);
    leg1->SetBorderSize(0);
    leg1->SetTextSize(18);
    leg1->SetTextFont(43);
    leg1->AddEntry(hhist[1][1], "NO assumed", "l");
    leg1->AddEntry(hhist[1][2], "IO assumed", "l");
    leg1->Draw();
