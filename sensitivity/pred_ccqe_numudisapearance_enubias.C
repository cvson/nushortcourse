...
void pred_ccqe_numudisapearance_enubias(){
    
    
    TFile *pfile = new TFile("op_eventmc_4sensitivity.root","READ");
    TFile *pfile_enbias = new TFile("op_eventmc_4sensitivity_enbias.root","READ");
    ...
    TH1D *hpred_osc_enbias[NDM23BIN][NTHETA23BIN];
    for (int idm=0; idm<NDM23BIN; ++idm) {
        for (int itheta=0; itheta<NTHETA23BIN; ++itheta) {
            ...
            hpred_osc_enbias[idm][itheta] = (TH1D*)pfile_enbias->Get(Form("hpred_osc_dmbin%d_thetabin%d",idm,itheta));
        }
    }
    TH1D *hpred_noosc = (TH1D*)pfile->Get("hpred_noosc");
    TH1D *hdata = (TH1D*)pfile->Get("hdata");
    TH1D *hdata_enbias = (TH1D*)pfile_enbias->Get("hdata");
    ...
    hdata->Draw("ep");
    hdata_enbias->SetMarkerStyle(8);
    hdata_enbias->SetMarkerColor(2);
    hdata_enbias->Draw("ep same");
    hdata->GetYaxis()->SetTitleOffset(1.5);
    hpred_noosc->SetLineWidth(2);
    hpred_noosc->SetLineColor(13);
    hpred_noosc->Draw("hist same");
    TLegend* leg0 = new TLegend(.40, .58, 0.85, .82);
    leg0->SetFillStyle(0);
    leg0->SetBorderSize(0);
    leg0->SetTextSize(18);
    leg0->SetTextFont(43);
    leg0->AddEntry(hdata, "Data", "ep");
    leg0->AddEntry(hdata_enbias, "Data w/ energy bias (50+/-5 MeV)", "ep");
    ...
    TH1D* hratio_enbias = (TH1D*)hdata_enbias->Clone("hratio_enbias");
    hratio_enbias->Sumw2();
    hratio_enbias->Divide(hpred_noosc);
