...
void plot_offaxis(){
    TF1 *fnu_energy_flux = new TF1("fnu_energy_flux","0.427*x/(1+pow([0]*x,2)/pow(0.139570,2))",0.,20.);
    fnu_energy_flux->SetParameter(0,0);
    new TCanvas;
    gStyle->SetOptStat(0);
    ...
    graph->Draw("f");
     //h1->DrawClone("");
    fnu_energy_flux->SetLineColor(kBlack);
    fnu_energy_flux->SetTitle("");
    fnu_energy_flux->SetLineWidth(3);
    fnu_energy_flux->DrawClone("same");
    
    fnu_energy_flux->SetParameter(0,7e-3);
    //UT orange
    ci = TColor::GetColor("#B45F04");
    fnu_energy_flux->SetLineColor(ci);
    fnu_energy_flux->DrawClone("same");
    
    fnu_energy_flux->SetParameter(0,14e-3);
    ...
    fnu_energy_flux->SetParameter(0,21e-3);
    //navy blue
    ...
    tlx3->Draw();
        gPad->Print("offaxis_energy_flux.eps");
