...
void plot(TH2D *sig, TH2D *bkg, TH2F *MVA, TString v0="var0", TString v1="var1",Float_t mvaCut=0){
   ...
   bkg->Draw("same");
    ...
    c->Print("data_circle_raw.eps");
   
    //c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_circle.pdf");
   MVA->Draw("CONT2 same");
	c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_circle.eps");
	//c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_circle.pdf");
	//c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_circle.C");
}


void PlotDecisionBoundary_circle( TString weightFile = "weights/TMVAClassification_test_boundary_circle_KNN.weights.xml",TString v0="var0", TString v1="var1", TString dataFileName = "data_circle.root")
{   
	weightFile = "weights/TMVAClassification_test_boundary_circle_"+methodStr+".weights.xml";
   //---------------------------------------------------------------
...
//Declaration of leaves types
   ...
   // write histograms
   //
   TFile *target  = new TFile( "TMVAPlotDecisionBoundary_circle.root","RECREATE" );
