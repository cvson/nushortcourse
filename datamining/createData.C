...
void plot( TString fname = "data.root", TString var0="var0", TString var1="var1" ) 
{
   ...
   TCanvas* c = new TCanvas( "c", "", 0, 0, 1000, 700 );
	c->SetFillColor(0);
   ...
   Int_t nbin = 500;
   TH2F* frameS = new TH2F( "DataS", "DataS", nbin, xmin, xmax, nbin, ymin, ymax*1.2 );
   TH2F* frameB = new TH2F( "DataB", "DataB", nbin, xmin, xmax, nbin, ymin, ymax*1.2 );
   ...
   // set style
   frameS->SetMarkerSize( 0.5 );
   frameS->SetMarkerColor( kAzure+7 );

   frameB->SetMarkerSize( 0.5 );
   frameB->SetMarkerColor( kOrange-3 );

   // legend
   frameS->SetTitle( var1+" versus "+var0+" for signal and background" );
   //frameS->GetXaxis()->SetTitle( var0 );
   //frameS->GetYaxis()->SetTitle( var1 );
	frameS->GetXaxis()->SetTitle("Variable 1");
	frameS->GetYaxis()->SetTitle("Variable 2");
	frameS->GetXaxis()->CenterTitle();
	frameS->GetYaxis()->CenterTitle();
   ...
   TString saveName=fname(0,fname.Length()-5);
	c->Print(saveName+"_InputVar.eps");
	c->Print(saveName+"_InputVar.pdf");
...
void create_lin(Int_t N = 2000)
{
   ...
   // output flie
   //TFile* dataFile = TFile::Open( "data_test_boundary.root", "RECREATE" );
	TFile* dataFile = TFile::Open( "data_test_boundary_antiCol.root", "RECREATE" );
   ...
   //Float_t xB[nvar] = { -0.0, -0.0 };
	Float_t xS[nvar] = {  0.8,  1.2 };
	Float_t xB[nvar] = { -0.8, -0.4 };
   Float_t dx[nvar] = {  1.0,  1.0 };
   TArrayD* v = new TArrayD( 2 );
   //Float_t rhoS =  0.21;
   //Float_t rhoB =  0.0;
	Float_t rhoS =  0.45;
	 Float_t rhoB =  -0.40;//antiCol
   ...
   //plot("data_test_boundary.root","var0","var1");
	plot("data_test_boundary_antiCol.root","var0","var1");
}

// create the data
void create_lin_org(Int_t N = 2000)
{
    const Int_t nvar = 2;
    Float_t xvar[nvar];
    Float_t weight;
