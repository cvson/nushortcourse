...
void plot(TH2D *sig, TH2D *bkg, TH2F *MVA, TString v0="var0", TString v1="var1",Float_t mvaCut=0){
   ...
   MVA->SetLineColor(kBlack);
   MVA->SetContour(1, contours);
   sig->SetMarkerColor(kAzure+7);
   bkg->SetMarkerColor(kOrange-3);
   sig->SetMarkerStyle(20);
   bkg->SetMarkerStyle(20);
   sig->SetMarkerSize(.6);
   bkg->SetMarkerSize(.6);
	sig->GetXaxis()->SetTitle("Variable 1");
	sig->GetYaxis()->SetTitle("Variable 2");
   ...
   bkg->Draw("same");
    ...
    legend->AddEntry(sig,"Signal","p");
    legend->AddEntry(bkg,"Background","p");
    legend->Draw("same");
    legend->SetBorderSize(1);
    legend->SetMargin( 0.3 );
    c->Print("data_3bumps_raw.eps");
   
    //c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_3bumps.pdf");
   MVA->Draw("CONT2 same");
	c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_3bumps.eps");
	//c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_3bumps.pdf");
	//c->Print(Form("DecisionBoundary%s_",MVA->GetTitle())+methodStr+"_3bumps.C");
}


void PlotDecisionBoundary_3bumps( TString weightFile = "weights/TMVAClassification_test_boundary_3bumps_KNN.weights.xml",TString v0="var0", TString v1="var1", TString dataFileName = "data_3Bumps.root")
{   
	weightFile = "weights/TMVAClassification_test_boundary_3bumps_"+methodStr+".weights.xml";
   //---------------------------------------------------------------
...
//Declaration of leaves types
   ...
   hb->SetXTitle(v0);
   hb->SetYTitle(v1);
   hs->SetMarkerColor(930);//tcolor->GetColor(0,191,255)
   hb->SetMarkerColor(931);
   ...
   // write histograms
   //
   TFile *target  = new TFile( "TMVAPlotDecisionBoundary_3bumps.root","RECREATE" );
