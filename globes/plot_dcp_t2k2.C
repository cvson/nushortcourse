...
void plot_dcp_t2k2(){
    char *pfilename[] = {
        "t2k2_final_nosmear_sensi_cp",
        "t2k2_final_wsmear_sensi_cp"
    };
    ...
    for (Int_t ihist=0; ihist<NHIST; ++ihist) {
        ...
        hhist[0][ihist]->GetYaxis()->SetRangeUser(0,6);
        hhist[0][ihist]->GetYaxis()->SetTitle("Sigma to exclude sin#delta_{CP}=0");
        ...
        if(ihist==0 || ihist==2) dcphist[3]->Draw("hist same");// if mass hierarchy is not known
        else dcphist[0]->Draw("hist same");// if mass hiearchy is known
        leg0->Draw();
        gPad->Print(Form("%s_comp.pdf",phistname[ihist]));
