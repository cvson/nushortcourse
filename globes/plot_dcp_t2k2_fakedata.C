...
void plot_dcp_t2k2_fakedata(){
    char *pfilename[] = {
        //"t2k2_final_wsmear_fakedata_cp"
        "t2k2_final_wsmear_fakedata_cp_poissonFluc"
    };
    ...
    new TCanvas;
    ci = TColor::GetColor(colorcode[0]);
    hhist[0][2]->SetLineColor(ci);
    
    ci = TColor::GetColor(colorcode[1]);
    hhist[0][3]->SetLineColor(ci);
    hhist[0][3]->GetYaxis()->SetRangeUser(0,12);
    hhist[0][3]->GetYaxis()->SetTitle("#sqrt{#Delta #chi^{2}}");
    hhist[0][3]->GetXaxis()->SetTitle("Test values of #delta_{CP} [rad]");
    titleStyle(hhist[0][3]);
    hhist[0][3]->Draw("hist");
    hhist[0][2]->Draw("hist same");
    TLegend* leg1 = new TLegend(.22, .62, 0.42, .92);
    leg1->SetFillStyle(0);
    leg1->SetBorderSize(0);
    leg1->SetTextSize(18);
    leg1->SetTextFont(43);
    leg1->AddEntry(hhist[0][2], "NO assumed", "l");
    leg1->AddEntry(hhist[0][3], "NO assumed", "l");
    leg1->Draw();
    gPad->Print(Form("%s_NOvsIO_comp.pdf",pfilename[0]));
